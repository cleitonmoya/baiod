...
T = len(Y)
results_path = "results/Y1_5300_2/"
...
Nomes_vet = ["Delta", "Prob_delta", "Eta", "P", "Beta"] # nome dos parâmetros que são vetores

Param_esc_burn = [Alpha_burn, Mu_burn]
Param_esc_final = [Alpha_final, Mu_final]
Param_vet_burn = [Delta_burn, Prob_delta_burn, Eta_burn, P_burn, Beta_burn]
Param_vet_final = [Delta_final, Prob_delta_final, Eta_final, P_final, Beta_final]
...
ax[0].axvline(burnin, linestyle='--', color='red')

ax[1].set_title(r"$\hat{\mu}$")
ax[1].plot(Mu, linewidth=0.5)
ax[1].axvline(burnin, linestyle='--', color='red', label='Burn-in')
_ = ax[1].legend(loc='upper right')

ax[2].set_title(r"$\hat{P}[\delta_t=1]$")
for j in Out:
    ax[2].plot(Prob_delta[:,j], label=f"t={j}", linewidth=0.5)
ax[2].axvline(burnin, linestyle='--', color='red')
_ = ax[2].legend(loc='upper center')
...
ax[0].axvline(burnin, linestyle='--', color='red')
_ = ax[0].legend(loc='upper center')
...
print("\n2. Diagnóstico de Gewek")
print("\n2.1. Antes dos saltos")
...
for i, vet in enumerate(Param_vet_burn):
    ...
    if len(t_nc)>0:
        if len(t_nc) > 10:
            print(f"\t{Nomes_vet[i]}: não convergiu para vários t")
        else:
            print(f"\t{Nomes_vet[i]}: não convergiu para t={t_nc}")
    else:
        print(f"\t{Nomes_vet[i]}: convergiu")

# 2.1.2 considerando somente t | delta_t = 1
...
# 2.2 Após burn-in e após os saltos
print("\n\n2.2.Após os saltos:")
...
# 1.2.1 considerando todos os t
print("\n2.2.1 Considerando todos os t:")
...
# 2.2.2 considerando somente t | delta_t = 1
print("\n2.2.2 Considerando somente t | delta_t=1:")
for i, vet in enumerate(Param_vet_final):
    vet = vet.round(decimals=2) # arredondamento para evitar erros
    G = np.array([geweke(vet[:,t], p1, p2) for t in t_delta1])
...
print("\n\n4. Tamanho da amostra efetiva (após burin-in")
print(f"4.1 Antes dos saltos (total = {tam_amostra_burn} observações):")
for i, par in enumerate(Param_esc_burn):
    nome = Nomes_esc[i]
    tam = ess(par)
...
print(f"\n 4.2 Após os saltos (total = {tam_amostra_final} observações):")
for i, par in enumerate(Param_esc_final):
    nome = Nomes_esc[i]
    tam = ess(par)
    print(f"\t{Nomes_esc[i]}: {tam:.0f}")
